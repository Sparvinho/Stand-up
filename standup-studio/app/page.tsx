...
export default function Home() {
  ...
  const toggleRecording = () => {
    // Om vi redan spelar in, stäng av mikrofonen
    if (isRecording) {
    ...
    recognitionRef.current = recognition; // Spara referensen så vi kan stoppa den
    ...
    recognition.onerror = (event: any) => {
      setIsRecording(false);
      // Ignorera "aborted" fel om användaren klickade på stopp självmant
      if (event.error !== 'aborted') {
        setRecordingError("Kunde inte höra vad du sa, eller så nekades mikrofonen.");
      }
    };
    ...
    <div className="min-h-screen bg-neutral-950 text-white p-4 md:p-10 pb-32">
      <div className="max-w-5xl mx-auto">
        ...
        <button
          ...
          className={`w-full relative overflow-hidden rounded-3xl p-10 mb-8 transition-all flex flex-col items-center justify-center min-h-[160px] border ${
            isRecording 
              ? 'bg-red-600/20 border-red-500/50 scale-[0.98]' 
              : 'bg-blue-600 hover:bg-blue-500 border-blue-500 hover:scale-[1.01] shadow-2xl shadow-blue-900/20'
          }`}
        >
          {isRecording ? (
            <>
              ...
              <Square size={40} className="text-red-500 mb-3 relative z-10 fill-red-500" />
              <h2 className="text-xl font-bold text-white mb-1 relative z-10">Klicka för att stoppa</h2>
              <p className="text-red-200 text-xs relative z-10">Mikrofonen lyssnar...</p>
            </>
          ) : (
            <>
